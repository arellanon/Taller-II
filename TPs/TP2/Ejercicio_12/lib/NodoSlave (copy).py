...
### Class server Remote
    ...
    def copiarMaster(self):
        ...
        Msg(socketMaster).send(self.id_nodo, ACCION_COPIAR, '', '')
        ...
        if accion == ACCION_REMOTO:        
            ...
            while len(comando):
                ...
                for path in modificar.keys():                    
                    ...
                    archivo.close()
                    time.sleep(0.01)
                    Msg(self.conn_client).send(self.id_nodo, MODIFICAR_ARCHIVO, path, datosArchivo)                    
                for path in agregar.keys():                    
                    archivo=open(path,'r')
                    datosArchivo = archivo.read()
                    archivo.close()
                    time.sleep(0.01)
                    Msg(self.conn_client).send(self.id_nodo, AGREGAR_ARCHIVO, path, datosArchivo)                                        
                for path in eliminar.keys():
                    Msg(self.conn_client).send(self.id_nodo, ELIMINAR_ARCHIVO, path, '')        
...
#                    paths= self.getPaths()
                """
                Msg(self.conn_client).send(self.id_nodo, ACCION_REMOTO, paths, str(salida))                                          
