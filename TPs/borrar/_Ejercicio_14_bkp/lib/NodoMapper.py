...
#        task = conn.recv_task()
        task_id, task_text = Mensaje(client_socket).recv()        
#        task_id, task_text = self.recv_msg_of_master(client_socket)
        task_text = unicode(task_text)
        print 'task_id: ' ,task_id, ' - type: ',type(task_id)
        # cerrar sockets
        client_socket.close()
        ...
        Mensaje(sock).send(task_id, str( index.dict() ) )
        sock.close()
