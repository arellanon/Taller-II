...
class NodoMapper(object):
    ...
    def _start_server(self):
        ...
        logging.info(
            'Iniciando la escucha en %s:%d', self.host, self.port)
        self._server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self._server_socket.setsockopt(
            socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self._server_socket.bind((self.host, self.port))
        self._server_socket.listen(self.listen)
    ...
    def _run_task(self):
        ...
        tokenizer = indexer.Tokenizer(self._stopwords(self.stopwsfile))
        index = indexer.Index(tokenizer)
        index.index_str(task_text)
        # enviar los resultados
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect(tuple(self.reducer))
